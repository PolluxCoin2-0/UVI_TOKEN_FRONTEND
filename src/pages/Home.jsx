...
const Home = () => {
  return (
    <div className="bg-black w-full h-full px-16 relative pb-12">
      ...
      {/* Video */}
      <div className="border-[1px] border-[#F6B63E] border-opacity-15 rounded-2xl bg-[#040510] h-[450px] mt-12 flex items-center justify-center relative">
          ...
          >
             <div className="flex flex-row space-x-10 items-center">
              {index === 0 ? (
                ...
                <p className="text-white text-2xl font-semibold">{index + 1}</p>
              )}
              <div>
                <p 
                 className={` ${ index === 0 ? 'text-black text-4xl font-bold ' : 'text-white text-2xl  font-semibold'}`}>{data.userName}</p>

                <p className={` pt-1 ${index === 0 ? 'text-black' : 'text-[#8C8B8B]'}`} >{data?.balance}</p>
            ...
            <div className={`${index ===0 ? 'mt-10' : "mt-0"}`}>
             ...
             <div className={` ${index === 0 ? 'flex flex-row space-x-8 mr-10' : 'flex flex-col items-end mr-5'}`}>
              ...
              {index === 0 && (
                <div  className="flex flex-row space-x-16">
                   <p className="text-[#FFC121] text-lg mt-2">Coin Holding</p>
                   <p className="text-white text-lg font-semibold mt-2">{data.TotalTransaction}</p>
                </div>
